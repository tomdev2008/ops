                    ...
                    <div class="col-md-6" style="width:100%">
                        <div class="panel panel-default">
                                <div class="panel-heading">
                                    <div class="text-muted bootstrap-admin-box-title"><strong>APP选择</strong></div>
                                </div>
                            <!-- 具体项目 -->
                                ...
                                <div class="controls" style="height:52px">
                                    <div style="margin-left:50px;margin-top:30px">
                                        ...
                                        <select data-placeholder="==请选择相应的项目==" name="app_name" style="width:250px;" class="chosen-select">
                                            <option value=""></option>
                                             <?php 
                                             foreach($get_project_name as $value){
                                                $select = $value->id == $projectid ? 'selected' :'';
                                            ?>
                                                <option value="<?php echo $value->id?>" <?php echo $select;?> >
                                                    <?php echo $this->disconfig_model->get_name_by_id($value->id);?>
                                                </option>
                                           <?php
                                            }
                                            ?>
                                        ...
                                        <?php echo form_error('env_name', '<div class="alert alert-error"><button class="close" data-dismiss="alert">&times;</button>', '</div>'); ?>
                                            <select data-placeholder="==请选择子项目==" style="width:350px;margin-right:50px" name="env_name" class="chosen-select-no-single">
                                                    <?php 
                                                        if ($app == "") {
                                                    ?>
                                                            <option value=""></option>
                                                    ...
                                                    ?>                                                        
                                                    <?php 
                                                        $project_info = $this->disconfig_model->get_name_by_project($projectid,$env);
                                                        $s = 0 ;
                                                        foreach ($project_info as $value) {
                                                            if ($value->server_name == $app) {
                                                                $s = 1;
                                                            }
                                                        }
                                                        if ($s == 1) {
                                                            foreach ($project_info as $value) {
                                                                $project_select = trim($value->server_name) == $app ? 'selected' : '';
                                                    ?> 
                                                        <option value="<?php echo trim($value->server_name)?>" <?php echo $project_select;?> >
                                                            <?php echo trim($value->server_name)."【".$value->server_alias_name."】";?>
                                                        </option>
                                                    <?php
                                                            }
                                                        }
                                                    ...
                                                    <?php
                                                            foreach ($project_info as $value) {
                                                    ?>
                                                        <option value="<?php echo $value->server_name?>">
                                                            <?php echo $value->server_name."【".$value->server_alias_name."】";?>
                                                        </option>
                                                    <?php
                                                            }
                                                        }
                                ...
                                <div class="bootstrap-admin-panel-content">
                                    <?php if($app != null && $env != null && $version != null){?>
                                        ...
                                        <div class="text-right" style="margin-top:-20px">
                                            <!-- <a href="javascript:;" onclick="copy_version('<?php echo $version_url;?>')">
                                          ...
                                          <!-- 复制版本按钮 -->
                                          <div class="btn-group">
                                            <button data-toggle="dropdown" class="btn btn-info btn-xs dropdown-toggle">复制版本
                                                <span class="caret"></span>
                                            </button>
                                            <ul class="dropdown-menu">
                                                <li>
                                                    <a href="javascript:;" onclick="copy_version_same_env('<?php echo $version_url;?>')">复制版本到当前环境</a>
                                                    <?php 
                                                        $version_url = $this->input->get('version') ? "" : "&version=".$default_version;
                                                    ?>
                                                    <a href="javascript:;" onclick="copy_version_other_env('<?php echo site_url("admin/disconfig/copy_to_other_env?".$add_url[1].$version_url);?>')">复制版本到其他环境</a>
                                                </li>
                                            </ul>
                                          </div>
                                            <button type="button" class="btn btn-info btn-xs" onclick="change_version('<?php echo site_url("admin/disconfig/change_version?".$add_url[1].$version_url);?>')">
                                                <span class="glyphicon glyphicon-pencil"></span>
                                                修改版本号
                                            </button>
                                            <a href="<?php echo site_url("admin/disconfig/downloadzip?$version_url");?>">
                                                <button type="button" class="btn btn-info btn-xs">
                                                    <span class="glyphicon glyphicon-pencil"></span>
                                                    批量下载
                                                </button>
                                    ...
                                    <?php foreach ($app_info as $value) {?>
                                        <tr>
                                            ...
                                            <td class="actions">
                                                <div style="float:right">
                                                    <a href="#">
                                                        <button class="btn btn-primary btn-xs" onclick="disconfig_update('<?php echo site_url('admin/disconfig/update/'.$value->config_id.'?env='.$env)?>')"><i class="glyphicon glyphicon-pencil"></i>
                                                        修改
                                                        </button></a>
                                        ...
                                        <tr>
                                            ...
                                            <td class="actions">
                                                <div style="float:right">
                                                    <a href="#">
                                                        <button class="btn btn-primary btn-xs" onclick="disconfig_update('<?php echo site_url('admin/disconfig/update/'.$value->config_id)?>')">
        ...
        <script type="text/javascript">
            ...
            $(".chosen-select").on('change', function (){
                // $.ajax({
                //   url: "disconfig/ajax_project?project_id="+$(this).val()+"&env_id="+env+"/",
                //   dataType: "json",
                //   beforeSend: function(){$('ul.chosen-select-no-single').empty();},
                //   success: function( servers ) {
                //     $chosenSelectSoSingle.empty()
                //     $chosenSelectSoSingle.append('<option value = ""></option>')
                ...
                //       $chosenSelectSoSingle.append('<option value="' + server_name + '">' +  server_name +' 【'+ alias_name + '】</option>');
                //       $(".btn").append('')
                //     })
                //         $chosenSelectSoSingle.trigger("chosen:updated");
                //      }
                // });
                var url = "/admin/disconfig?env=" + env + "&project_id=" + $(this).val()
                $(location).attr('href',url);
            })
            // url跳转
            $chosenSelectSoSingle.on('change',function(){
                var project_id = $(".chosen-select").val()
                var url = "/admin/disconfig?env=" + env + "&project_id=" + chosen + "&app=" + $(this).val()
                $(location).attr('href',url);
            })
            //提交控制
            ...
            // 复制版本配置（确定操作）
            function copy_version_same_env(info){
                layer.confirm('您确定要复制该版本吗？', {
                    ...
                    $.ajax({
                        ...
                        error: function () {
                            layer.msg('复制失败', {time: 1000,icon: 1});
                        }
                    })
                });
            }
            function copy_version_other_env(url){
                layer.open({
                    title: false,
                    type: 2,
                    skin: 'layui-layer-demo', //样式类名
