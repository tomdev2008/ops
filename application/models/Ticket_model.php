...
class Ticket_model extends CI_Model {
	...
	public function get_ticket_list($level_id,$page,$pagenum) {
		...
		$user_id = $this->session->userdata('u_id');
		if ($user_level_id != '2') {
			//$where_sql = "where ticket_user_level ='".$user_level_id."'";
			if ($level_id) {
				$ticket_sql_where = $level_id ? " where ticket_user_level = ".$level_id."" : "";
				$ticket_sql_where = $ticket_sql_where." and submitter ='".$user_id."' or (status = 4 and ticket_user_level = ".$level_id.")";
			}
			else if ($level_id == NULL) {
				$ticket_sql_where = "where submitter ='".$user_id."' or status = 4 ";
			}
			//if ($page == NULL) {
				//$ticket_sql_where = $ticket_sql_where." OR sid = 1";
			//}
			//$ticket_sql_where = $ticket_sql_where." OR sid = 1";							
		}
		else if ($user_level_id == '2'){
		...
		//$ticket_sql_where = $ticket_sql_where." and pid = 1";	
		//$query = $this->db->query('select * from ops_ticket'.$ticket_sql_where);
		//echo $ticket_sql_where;
		...
		//}
		//else if ($user_level_id == '1'||$user_level_id == '2'){
			//$query = $this->db->query("select * from ops_ticket ".$ticket_sql_where." order by status desc,opr_time DESC limit ".$page.",".$pagenum);
		//}
		$row = $query->result();
	...
	public function Email_to_ops($contents,$email,$title,$name){
		...
		$link = "<a href='http://ops.xkeshi.so/ticket/'>http://ops.xkeshi.so/ticket/</a>";
		$footer = "
		<br>
		<hr/>
		<p>
