...
class Disconfig_model extends CI_Model {
	...
	public function get_version_by_appid_env($app,$env){
		$query = $this->db->query("select distinct version from ops_disconfig where app_id= '".$app."' and env_id= '".$env."' order by version desc");
		$data = $query->result();
		return $data;
	}
	public function get_name_by_project($project_id){
		$query = $this->db->query("select distinct server_name,server_alias_name from ops_app_server where server_env = '4' and server_project = '".$project_id."'");
		$row = $query->result();
		return $row;
	}
	...
	public function get_info_by_app_env_version($app,$env,$version){
		$query = $this->db->query("select * from ops_disconfig where app_id= '".$app."' and env_id= '".$env."' and version= '".$version."' order by sensitive_status desc,create_time desc ");
		$data = $query->result();
		return $data;
	}
	public function get_conf_data_by_id($id){
		$this->db->select('name,value,app_id,sensitive_status');
